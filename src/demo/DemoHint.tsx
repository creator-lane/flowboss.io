...
interface DemoHintProps {
  ...
  emoji?: string;
  /**
   * "default" — full billboard with icon, body, and CTA button (~200px tall).
   * "compact" — single thin strip, title + inline CTA link only (~44px tall).
   *   Use compact in sticky sidebars or anywhere a tall card would push the
   *   actual feature below the fold.
   */
...
export function DemoHint({
  ...
  // Compact: thin one-line strip for sticky sidebars / tight columns.
  // Renders as an inline badge → title → CTA link → dismiss X. No body.
  if (size === 'compact') {
    return (
      <div
      ...
      >
        <div
          className={`relative rounded-xl p-[1.5px] bg-gradient-to-r ${t.border} shadow-md ${t.glow}`}
        >
          <div className="rounded-[10px] bg-white px-3 py-2 dark:bg-neutral-900">
            <div className="flex items-center gap-2.5 min-w-0">
              <div
                className={`flex-shrink-0 w-6 h-6 rounded-md ${t.iconBg} flex items-center justify-center text-white shadow-sm`}
              >
                {emoji ? (
              ...
              <div className="flex-1 min-w-0 flex items-center gap-2 flex-wrap">
                <span
                  className={`text-[9px] font-extrabold uppercase tracking-[0.12em] bg-gradient-to-r ${t.badge} bg-clip-text text-transparent whitespace-nowrap`}
                >
                  Demo
                </span>
                ...
                <a
                  href={ctaHref}
                  className={`text-xs font-bold bg-gradient-to-r ${t.badge} bg-clip-text text-transparent hover:underline whitespace-nowrap`}
                >
                  {cta}
                </a>
              </div>
              <button
                ...
                className="flex-shrink-0 p-1 rounded-md text-gray-400 hover:text-gray-700 hover:bg-gray-100 dark:hover:bg-white/10 dark:hover:text-gray-200"
              >
